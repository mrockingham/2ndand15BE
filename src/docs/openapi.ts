  ...
  paths: {
    ...
    '/standings': {
      get: {
        ...
        summary: 'Get provider-backed NFL standings',
        tags: ['Standings'],
        description:
          'DB-only public read of the latest ingested official provider snapshot. PRE and REG are supported when ingested; POST is not published by the current provider. Rows preserve provider conference order; league view uses record, wins, point differential, then provider order.',
        parameters: [
          {
            name: 'season',
            in: 'query',
          ...
          { name: 'conference', in: 'query', schema: { type: 'string', enum: ['AFC', 'NFC'] } },
          {
            name: 'division',
            in: 'query',
            schema: { type: 'string', enum: ['East', 'North', 'South', 'West'] },
          },
          { name: 'teamId', in: 'query', schema: { type: 'string', format: 'uuid' } },
        ],
        responses: {
          '200': {
            description: 'Frontend-ready grouped standings and snapshot metadata.',
            content: {
              'application/json': { schema: { $ref: '#/components/schemas/StandingsResponse' } },
            },
          },
          '400': { $ref: '#/components/responses/ValidationError' },
          '404': { $ref: '#/components/responses/NotFoundError' },
    ...
    schemas: {
      ...
      StandingTeam: {
        ...
        properties: {
          ...
          ties: { type: ['integer', 'null'] },
          winPercentage: { type: ['number', 'null'] },
          homeWins: { type: ['integer', 'null'] },
          homeLosses: { type: ['integer', 'null'] },
          homeTies: { type: ['integer', 'null'] },
          awayWins: { type: ['integer', 'null'] },
          awayLosses: { type: ['integer', 'null'] },
          awayTies: { type: ['integer', 'null'] },
          divisionWins: { type: ['integer', 'null'] },
          divisionLosses: { type: ['integer', 'null'] },
          divisionTies: { type: ['integer', 'null'] },
          conferenceWins: { type: ['integer', 'null'] },
          conferenceLosses: { type: ['integer', 'null'] },
          conferenceTies: { type: ['integer', 'null'] },
          nonConferenceWins: {
            type: ['integer', 'null'],
            description: 'Null: current provider does not supply this split.',
          },
          nonConferenceLosses: { type: ['integer', 'null'] },
          nonConferenceTies: { type: ['integer', 'null'] },
          pointsFor: { type: ['integer', 'null'] },
          pointsAgainst: { type: ['integer', 'null'] },
          pointDifferential: { type: ['integer', 'null'] },
          streakType: { type: ['string', 'null'], enum: ['W', 'L', 'T', null] },
          streakLength: { type: ['integer', 'null'] },
          streakDisplay: { type: ['string', 'null'] },
          lastFiveWins: {
            type: ['integer', 'null'],
            description: 'Null: current provider does not supply last-five form.',
          },
          lastFiveLosses: { type: ['integer', 'null'] },
          lastFiveTies: { type: ['integer', 'null'] },
          lastFiveDisplay: { type: ['string', 'null'] },
          conferenceRank: { type: ['integer', 'null'] },
          playoffSeed: { type: ['integer', 'null'] },
          divisionRank: { type: ['integer', 'null'] },
          leagueRank: { type: ['integer', 'null'] },
          clinchedCode: { type: ['string', 'null'] },
          eliminated: { type: ['boolean', 'null'] },
        },
      },
      StandingsGroup: {
        type: 'object',
        ...
        properties: {
          key: { type: 'string' },
          label: { type: 'string' },
          teams: { type: 'array', items: { $ref: '#/components/schemas/StandingTeam' } },
          children: { type: 'array', items: { $ref: '#/components/schemas/StandingsGroup' } },
        },
      },
      StandingsResponse: {
        type: 'object',
        ...
        properties: {
          data: {
            type: 'object',
            required: ['season', 'seasonType', 'view', 'groups'],
            properties: {
              season: { type: 'integer' },
              seasonType: { type: 'string', enum: ['PRE', 'REG', 'POST'] },
              view: { type: 'string', enum: ['division', 'conference', 'league'] },
              groups: { type: 'array', items: { $ref: '#/components/schemas/StandingsGroup' } },
            },
          },
          meta: {
            type: 'object',
            required: ['availableViews', 'availableSeasonTypes', 'provider', 'updatedAt'],
            properties: {
              availableViews: {
                type: 'array',
                items: { type: 'string', enum: ['division', 'conference', 'league'] },
              },
