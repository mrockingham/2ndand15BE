...
/**
 * M31C: a bounded repair/backfill for embed eligibility on highlights that
 * already exist -- never a season-wide scan. Only calls
 * `/highlights/geo-restrictions/{id}`, once per row lacking a decision (or
 * every row, with `--force`); it never re-fetches `/highlights` metadata.
 * Mirrors `game-highlights-sync.ts`'s bounded-batch CLI conventions.
 */
const MAXIMUM_BATCH_GAMES = 20;

const singleGameArguments = z.object({ gameId: z.uuid() }).strict();
const multiGameArguments = z.object({ gameIds: z.string().min(1) }).strict();
...
async function main(): Promise<void> {
  ...
  try {
    ...
    for (const gameId of gameIds) {
      results.push(await service.refreshEmbedEligibility(gameId));
    }
    const summary = {
      gamesChecked: results.length,
      totalDbHighlights: results.reduce((sum, r) => sum + r.dbHighlightCount, 0),
      totalGeoRequests: client.getRequestCount() - requestCountBefore,
    };
...
async function resolveGameIds(
  ...
  const multi = multiGameArguments.safeParse(raw);
  if (multi.success) {
    const gameIds = multi.data.gameIds.split(',').map((value) => value.trim());
    const parsed = z.array(z.uuid()).max(MAXIMUM_BATCH_GAMES).safeParse(gameIds);
    if (!parsed.success) {
      throw new Error(
        `--gameIds must be a comma-separated list of up to ${String(MAXIMUM_BATCH_GAMES)} UUIDs.`,
      );
    }
    return parsed.data;
  }
  ...
  if (!window.success) {
    throw new Error(
      'Usage: --gameId=<uuid> OR --gameIds=<uuid,uuid,...> OR --season=<year> --seasonType=<PRE|REG|POST> [--week=<number>]',
    );
  }
...
void main().catch((error: unknown) => {
  process.stderr.write(
    `${error instanceof Error ? error.message : 'Embed eligibility backfill failed.'}\n`,
  );
  process.exitCode = 1;
});
