...
try {
  ...
  process.stderr.write(
    `${JSON.stringify({
      error: {
        code: syncError?.code ?? providerError?.code ?? 'CURRENT_GAME_DETAILS_SYNC_FAILED',
        message:
          syncError?.message ??
          providerError?.message ??
          'Current-game detail synchronization failed; inspect private operational logs.',
...
function parseAction(value: string | undefined): Action {
  ...
  throw new Error('Expected current-game detail action: verify or sync.');
}
